...
```{r}
table(real_estate_df$city)
table(real_estate_df$year)
table(real_estate_df$year, real_estate_df$city)
table(real_estate_df$month)
```
...
p_dic_2012 <- round(month_year_counts["Dec-2012"]/length(date),3)
print(p_dic_2012) 
```
...
Riportiamo anche il boxplot del totale vendite (**volume**) in funzione dell'anno in modo da vedere come si è comportata l'azienda nel tempo:


```{r}
ggplot() +
    geom_boxplot(aes(x = as.factor(year), y = volume, color = as.factor(year)),
                 fill = "lightblue") +
    labs(title = "BP di volume per anno",
         x = "Anno",
         y = "Volume",
         color = NULL) +
    theme_minimal()+
...
Da questo boxplot possiamo vedere come la società sia cresciuta negli anni aumentano la mediana di **volume** ma ha aumentato anche la variabilità dei suoi ricavi nei diversi mesi, infatti il "box" si è allargato costantemente.
